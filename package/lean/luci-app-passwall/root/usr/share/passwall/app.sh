...
SmartDNS() {
	m=$(sed -n '/bind-tcp: 6053/=' /etc/smartdns/custom.conf)
	let 'm=m+1'
	sed -i $m'c bind: '$1' -group oversea' /etc/smartdns/custom.conf
	let 'm=m+1'
	sed -i $m'c bind-tcp: '$1' -group oversea' /etc/smartdns/custom.conf
	...
	uci set smartdns.@smartdns[0].seconddns_server_group='oversea'
	uci commit smartdns
	/etc/init.d/smartdns reload
}
 
start_dns() {
	[ "$DNS_MODE" != "SmartDNSx" ] && SmartDNS "8653"
	...
	SmartDNSx)
		...
		SmartDNS ${DNS_PORT}
		echolog "DNS：使用SmartDNS("${DNS_PORT}")服务器解析域名(Oversea)..."
	;;
	dns2socks)
...
start() {
	[ ! -f "/var/etc/passwall_server.include" ] && echo "#!/bin/sh" > /var/etc/passwall_server.include
	if [ -f "/etc/init.d/shadowsocksr" ]; then
		[ ! -f "/var/etc/shadowsocksr.include" ] && echo "#!/bin/sh" > /var/etc/shadowsocksr.include
